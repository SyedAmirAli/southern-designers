            ...
            <div class="dropdown ms-sm-3 header-item topbar-user">
                <button type="button" class="btn btn-light rounded-0 border-0" data-bs-toggle="dropdown">
                    <span class="d-flex align-items-center">
                        ...
                        @endphp
                        <img class="rounded-circle img-fit lazyload" data-src="{{ $image ? (file_exists($image) ? asset($image) : $image) : asset('backend/images/avatar/default/user.jpg') }}" width="50" height="50"
                            alt="Header Avatar">
                        <span class="ms-2 text-start d-md-block d-none">
                            <span class="fs-15 d-block lh-1 ms-1">{{ Auth::user()->name }} ({{ Auth::user()->user_name }})</span>
                            <span class="ms-1 fs-12">{{ Auth::user()->email }}</span>
                        </span>
                    </span>
                </button>
