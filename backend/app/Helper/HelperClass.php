...
class HelperClass
{
    public static function checkOwnerShip(string|int $created_by, bool $redirect = true){
        if(UserManagement::role('admin')) return true;

        if(Auth::id() == $created_by) return true;

        if($redirect) return redirect()->back()->withErrors("You don't have a permission to do this!");
    ...
    */
    ...
    public static function resourceDataView($model, array $data_column, NULL|array $image_column, string|array $route_path, $resources = null){
        ...
        if (request()->ajax()) {
            ...
            $datatables_eloquent->addColumn('checkbox', function ($row) {
                ...
                if(ResourceAppManager::hasChangePermission(true, $row->created_by)){
                    return $checkbox;
                }

                return "<div><input type='checkbox' class='custom-control-input multi_checkbox' disabled /><label for='' class='custom-control-label'></label></div>";
            });
    ...
    string|NULL $file_path = null, array|string $path, array|NULL $multiple_file = null, $resources = null){
        ...
        $creation_data['created_by'] = Auth::id();
    ...
    public static function resourceDataEdit(string $table_name, string $id, string|array $path, $resources = null){
        ...
        if (request()->ajax() && request('status') == 'true') {
            ...
            $update = fn() => DB::table($table_name)->where('id', $id)->update(['status' => !$data->status]);
    ...
    public static function resourceDataDelete(string $table_name, $request, string $id, array|NULL $files, array|NULL $multiple_file = null, $resources = null){
        ...
        if($request->id){
            ...
            foreach ($delete_all_data as $all_data) {
                if(!UserManagement::role('admin')){
                    if(isset($all_data->created_by)){
                        if(Auth::id() != $all_data->created_by){
